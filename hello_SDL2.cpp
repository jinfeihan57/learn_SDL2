...
int main(int argc, char *argv[])
{
    ...
    while (!quit) {
        ...
        if (keyStatus[SDL_SCANCODE_Q] == SDL_PRESSED) {
            ...
            index = 0;
            current = jump2;
        } else if (keyStatus[SDL_SCANCODE_RIGHT] == SDL_PRESSED) {
            tigerHeadx += 1;
            flip = SDL_FLIP_NONE;
        } else if (keyStatus[SDL_SCANCODE_LEFT] == SDL_PRESSED) {
            tigerHeadx -= 1;
            flip = SDL_FLIP_HORIZONTAL;
        } else if (keyStatus[SDL_SCANCODE_DOWN] == SDL_PRESSED) {
            tigerHeady += 1;
        } else if (keyStatus[SDL_SCANCODE_UP] == SDL_PRESSED) {
            tigerHeady -= 1;
        }
